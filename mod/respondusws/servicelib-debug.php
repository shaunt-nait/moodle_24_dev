...
function RWSPreloginCAS($username, $password, $cas_failed)
{
	...
	if ($RWS_ENABLE_CAS_MULTI || $cas_plugin->config->multiauth) {
		if (isset($username)) {
			$service_url .= "&rwsuser=";
	...
	while (strlen($lt) == 0
	  ...
	  || strlen($reset) == 0
		...
		if (strlen($warn) == 0) {
			$start = stripos($body, "name=\"warn\"", $next);
			if ($start === FALSE || $start > $end)
				$start = stripos($body, "name = \"warn\"", $next);
			if ($start === FALSE || $start > $end)
				$start = stripos($body, "name=\'warn\'", $next);
			...
			if ($start !== FALSE && $start < $end) {
				...
				if ($pos !== FALSE && $pos < $end) {
					...
					while ($pos < $end && $body[$pos] != "\"" && $body[$pos] != "\'")
						$pos++;
					$end = $pos;
					$warn = substr($body, $start, $end - $start);
				}
			}
		}
		if (strlen($execution) == 0) {
			$start = stripos($body, "name=\"execution\"", $next);
			if ($start === FALSE || $start > $end)
				$start = stripos($body, "name = \"execution\"", $next);
			if ($start === FALSE || $start > $end)
				$start = stripos($body, "name=\'execution\'", $next);
			...
			if ($start !== FALSE && $start < $end) {
				...
				if ($pos !== FALSE && $pos < $end) {
					...
					while ($pos < $end && $body[$pos] != "\"" && $body[$pos] != "\'")
						$pos++;
					$end = $pos;
					$execution = substr($body, $start, $end - $start);
				}
			}
		}
		if (strlen($reset) == 0) {
			$start = stripos($body, "name=\"reset\"", $next);
			if ($start === FALSE || $start > $end)
				$start = stripos($body, "name = \"reset\"", $next);
			if ($start === FALSE || $start > $end)
				$start = stripos($body, "name=\'reset\'", $next);
			...
			if ($start !== FALSE && $start < $end) {
				...
				if ($pos !== FALSE && $pos < $end) {
					...
					while ($pos < $end && $body[$pos] != "\"" && $body[$pos] != "\'")
						$pos++;
					$end = $pos;
					$reset = substr($body, $start, $end - $start);
				}
			}
		}
		$pos = $next + 1;
	...
	if (isset($warn)) {
		$post_fields .= "&warn=";
		$post_fields .= urlencode($warn);
	}
	if (isset($execution)) {
		$post_fields .= "&execution=";
		$post_fields .= urlencode($execution);
	}
	if (isset($reset)) {
		$post_fields .= "&reset=";
		$post_fields .= urlencode($reset);
	}
	...
	if ($rwscas == "1") { 
		...
		if (isset($ticket)) {
			...
			if (phpCAS::checkAuthentication())
				exit; 
			if (isset($rwsuser) && isset($rwspass))
...
function RWSActionDnloadFile()
{
	...
	if ($start !== FALSE) {
		...
		if (RWSFloatCompare($CFG->version, 2011120500.00, 2) >= 0) { 
			$component = clean_param(array_shift($parts), PARAM_COMPONENT);
			$filearea = clean_param(array_shift($parts), PARAM_AREA);
		}
		else { 
